...
class DumpWatcher implements Watcher {
  ...
  handleContent(entry: any, action: "add" | "view" | "index", id?: string) {
    const queries = {
      mysql: {
        ...
        index: () =>
          this.connection.query("SELECT * FROM observatory_entries WHERE type = 'dump' ORDER BY created_at DESC"),
      },
      postgres: {
        add: () =>
          this.connection.query(
  ...
  public async getView(req: Request, res: Response): Promise<Response> {
    try {
      const data = await this.handleContent(null, "view", req.params.dumpId);
      return res.status(200).json(data);
    } catch (error) {
      console.error("Error getting view from DumpWatcher", error);
      return res.status(500).json({ error: "Internal Server Error" });
    }
  }
}
