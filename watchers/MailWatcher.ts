...
class MailWatcher implements Watcher {
  ...
  async addContent(content: unknown): Promise<void> {
    const entry = {
      uuid: uuidv4(),
      batch_id: uuidv4(),
      family_hash: uuidv4(),
  ...
  handleContent(entry: any, action: "add" | "view" | "index", id?: string) {
    const queries = {
      mysql: {
        ...
        index: () =>
          this.connection.query("SELECT * FROM observatory_entries WHERE type = 'mail' ORDER BY created_at DESC"),
      },
      postgres: {
        add: () =>
          this.connection.query(
  ...
  async getView(req: Request, res: Response): Promise<Response> {
    try {
      const data = await this.handleContent(null, "view", req.params.mailId);
      return res.status(200).json(data);
    } catch {
      return res.status(500).json({ error: "Internal Server Error" });
    }
