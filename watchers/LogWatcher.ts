...
class LogWatcher implements Watcher {
  ...
  handleContent(entry: any, action: "add" | "view" | "index", id?: string) {
    const queries = {
      mysql: {
        ...
        index: () =>
          this.connection.query("SELECT * FROM observatory_entries WHERE type = 'log' ORDER BY created_at DESC"),
      },
      postgres: {
        add: () =>
          this.connection.query(
  ...
  async getView(req: Request, res: Response): Promise<Response> {
    try {
      const data = await this.handleContent(null, "view", req.params.logId);
      return res.status(200).json(data);
    } catch {
      return res.status(500).json({ error: "Internal Server Error" });
    }
