...
class ExceptionWatcher implements Watcher {
  ...
  handleContent(entry: any, action: "add" | "view" | "index", id?: string) {
    const queries = {
      mysql: {
        ...
        index: () =>
          this.connection.query("SELECT * FROM observatory_entries WHERE type = 'exception' ORDER BY created_at DESC"),
      },
      postgres: {
        add: () =>
          this.connection.query(
  ...
  public async getView(req: Request, res: Response) {
    try {
      const data = await this.handleContent(null, "view", req.params.exceptionId);
      return res.status(200).json(data);
    } catch (error) {
